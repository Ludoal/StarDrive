...
namespace Ship_Game
{
    ...
    public sealed class AutomationScreen : GameScreen
    {
        ...
        const float EmpireBoxH = 160f, ColonizationBoxH = 156f, ConstructionBoxH = 139f, // +26: Auto-explore split adds a row (Send New Explorers)
                    TradeBoxH = 204f, NotificationsBoxH = 500f, PriorityBoxH = 330f; // Notifications frame must be >= its content height (timer 28 + label + ~13 rows*pitch); the UIList isn't clipped so a short frame lets Inhibition spill (bench 486)
