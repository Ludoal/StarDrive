...
namespace Ship_Game
{
    public partial class BlueprintsScreen : GameScreen
    {
        ...
        public void AfterBluprintsDelete(BlueprintsTemplate template)
        {
            ...
            // the editor may still be linked to the deleted plan: a dangling name would be saved
            if (LinkedTo == template.Name)
                SetLinkedTo("");
        } 

        // called with the plan that just lost its link, so the colonies holding it read the
        // unlinked template; the editor's own link is cleared in AfterBluprintsDelete
