...
namespace Ship_Game
{
    // Ludoal fork: the Stats+ layout shared by the Colony facilities panel AND the Blueprints
    // hover panel. ⚠ ONE geometry, tuned to the millimetre for a 1440px frame - never fork it
    // into a second copy. All helpers are pure formatting: they take the font and the block width
    // and hold no screen state, and the column pivots live in SPCols so two screens can draw at
    // once without stepping on shared statics.
    ...
    public static class StatsPlusLayout
    {
        ...
        // fixed pixels: the tab is 2/3 of a 2/3-screen menu, so its width follows the resolution.
        // Ratios calibrated on 1080p (105/165/220/280 out of 350px); narrower frames compress
        ...
        public static SPCols SPSetColumns(Font font, float blockW)
        {
            ...
            // ("Net growth (M / turn)") at any width (bench 285)
        ...
        // every number of a column shares one decimal point
        public static void SPNum(SpriteBatch batch, Font font, float pivotX, float y, string s, Color color)
        {
            ...
            if (dot < 0)
            {
                ...
                // whole string would push suffixed values ("8  (~25 turns)", "+0") onto their labels.
                dot = 0;
        ...
        {
            // headers CENTRE on their pivot (bench 284): a decimal column's visual middle
